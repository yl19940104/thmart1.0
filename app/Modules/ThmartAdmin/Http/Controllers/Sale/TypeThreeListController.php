...
class TypeThreeListController extends Controller
{
    public function index(Request $request){
        ...
        if (isset($param) && $param) {
            //搜索条件有促销价的情况
            $condition = ['sku.isDelete'=>0, /*['startTime', '<=', time()], */['endTime', '>=', time()], 'item.isDelete'=>0, 'item.audited'=>1];
            //搜索条件无促销价的情况
            ...
            $res = DB::table('sku')
                ->select('itemSalePrice.id', 'item.goodsNumber', 'sku.skuNumber', 'startTime', 'endTime', 'category.title as categoryTitle', 'item.title', 'item.subTitle', 'sku.price', 'itemSalePrice.salePrice', 'itemSalePrice.type', 'brand.name as brandName', 'item.id as itemId', 'itemSalePrice.amount')
            ...
            foreach ($res as $k => &$v) {
                ...
                $v->price *= 0.01;
                $v->salePrice *= 0.01;
                //如果改sku没有促销价，那么促销价设为空,开始时间为空，结束时间为空
                if ($v->salePrice == 0) {
                    $v->salePrice = '';
                    $v->startTime = '';
                    $v->endTime = '';
                }
                //如果该sku有促销价和团购价，那么清除促销价和团购价数据
                if ($v->type != 3) {
                    $v->id = null;
                    $v->startTime = null;
                    $v->endTime = null;
        ...
        return view('thmartAdmin::Sale/typeThreeList', compact('compactData'));
    }
}